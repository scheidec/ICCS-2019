...
Check that 1st cit norm indicator recoded correctly - count of original and recoded variable categories
```{r}
tbl %>% count(BS3B1,BS3B1_binary) 
```

Use "select" to rename mutated variable and display first five lines of dataframe for 1st mutated indicator
```{r}
tbl %>% select(obey=BS3B1_binary) %>% head
```

Copy recode for 1st cit norm indicator and - add in additonal indicators. NOTE: BS3B12 does not mutate correctly because of typo in string variable.
```{r}
tbl <-tbl %>% 
...
Confirm that mutate coded correctly
```{r}
tbl %>% count(BS3B1,BS3B1_bin)
tbl %>% count(BS3B2,BS3B2_bin)
tbl %>% count(BS3B3,BS3B3_bin)
tbl %>% count(BS3B4,BS3B4_bin)
tbl %>% count(BS3B5,BS3B5_bin)
tbl %>% count(BS3B6,BS3B6_bin)
tbl %>% count(BS3B8,BS3B8_bin)
tbl %>% count(BS3B9,BS3B9_bin)
tbl %>% count(BS3B10,BS3B10_bin)
tbl %>% count(BS3B11,BS3B11_bin)
tbl %>% count(BS3B13,BS3B13_bin)
```

Count table of BS3B12 to identify source of "mutate" error - console output shows that string text of 1st category spelled incorrectly: "1) not importnat""  i.e. "a" and "n" transposed.
```{r}
tbl %>% count(BS3B12) 
```
...
tbl <-tbl %>% 
  ...
  "important"=c("(3) somewhat important", "(4) very important")))
```
Confirm that recoded correctly:
```{r}
tbl %>% count(BS3B12,BS3B12_bin) 
```


Use "select" to rename 2 mutated variables and display first five lines of dataframe to confirm if coded correctly - listed first two vars of 1999 descending means (IJCS article)
```{r}
tbl %>% select(obey=BS3B1_bin,vote=BS3B2_bin) %>% as_tibble %>% head
```

Use "select" to rename all 12 mutated variables and display first five lines of dataframe to confirm if coded correctly - listed by 1999 descending means (IJCS article). Notes: Indicate name "local" used in IJCS article cannot be used here for mutate select command because dataset already has with this name, so command below uses var name "local_" to be recoded alter
```{r}
tbl %>% select(obey=BS3B1_bin,rights=BS3B11_bin,local_=BS3B9_bin,local=BS3B4_bin,envir=BS3B13_bin,vote=BS3B2_bin,history=BS3B6_bin,respect=BS3B10_bin,news=BS3B8_bin,protest=BS3B5_bin,discuss=BS3B12,party=BS3B3_bin) %>% 
  as_tibble %>% head
```


Base R Cit norms recode as reference - next few chunks of code replicate above tidyverse approach in base R - marked as eval=FALSE so doesn't run.
```{r, eval=FALSE}
...
Base R checking if cit norms are factors
```{r, eval=FALSE}
is.factor(tbl_reduced$BS3B1)
summary(tbl_reduced$BS3B1)
```
Base R, mapping new dichotomous variable
```{r, eval=FALSE}
...
```
Base R checking recode - command useful for items that have the same coding
```{r}
table(tbl_reduced$BS3B1,tbl$BS3B1)
```

XX
Unique id per observation creation across binded country files
```{r}
id <- 1:93882; tbl <- cbind(id,tbl)
```

Same type of variable creation, except without noting precise number range ("n" instead)
```{r}
n <- dim(tbl)[1]; id <- 1:n; tbl <- cbind(id,tbl)
```

XX

Select variables for latent gold analysis
select(id,COUNTRY, JO add cit norm variablesXX )
...
XX
For ICCS year variable - 

RdM code for exporting data: 
