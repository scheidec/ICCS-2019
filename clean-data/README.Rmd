...
XX
Attempt to load all country files at once - unsuccessful
```{r, eval=FALSE}
...
Bind all 1999 files. Note, total observations of resulting tbl (93,882) concur with xls documentation of expected total n
```{r}
...
Cit norms recode for 1st cit norm indicator
```{r}
tbl <-tbl %>% 
...
Copy prior code to edit - add in additonal indicators - cit norms recode for all cit norm indicators
```{r}
tbl <-tbl %>% 
...
XX
Continue cit norms recode for all indicators:
XX

Cit norms recode with base R (RdM)
```{r}
...
Checking if cit norm vars are factors
```{r}
...
Checking recode - command useful for items that have the same coding
```{r}
